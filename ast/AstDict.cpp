...
void AstDict::compute_hash()
{
    ...
    for (auto it : vec)
    {
        Expression* e1 = it.first;
        this->hash_c += e1->get_hash();
        Expression* e2 = it.second;
        this->hash_c += e2->get_hash();
    }
    for (auto it : dict)
    {
        string s = it.first;
...
{
    string res = "{";
    for (unordered_map<string, Expression*>::iterator it = dict.begin(); it != dict.end();)
    {
        res += it->first;
        res += ": ";
...
const vector<pair<Expression*, Expression*> >& AstDict::get_vec()
{
    return vec;
}
